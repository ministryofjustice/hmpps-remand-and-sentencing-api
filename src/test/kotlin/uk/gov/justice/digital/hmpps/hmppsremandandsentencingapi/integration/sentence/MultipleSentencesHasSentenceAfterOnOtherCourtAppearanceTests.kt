...
class MultipleSentencesHasSentenceAfterOnOtherCourtAppearanceTests : IntegrationTestBase() {

  @Test
  fun `when sentences on other case is after return true`() {
    ...
    webTestClient
      .get()
      .uri {
        it.path("/sentence/has-sentences-after-on-other-court-appearance")
  ...
  fun `when sentence on same case same appearance is after return false`() {
    ...
    webTestClient
      ...
      .uri {
        it.path("/sentence/has-sentences-after-on-other-court-appearance")
          .queryParam("sentenceUuids", sentence.sentenceUuid, otherSentence.sentenceUuid)
          .build()
      }
      .headers {
        it.authToken(roles = listOf("ROLE_REMAND_AND_SENTENCING__REMAND_AND_SENTENCING_UI"))
  ...
  fun `no token results in unauthorized`() {
    ...
    webTestClient
      ...
      .uri {
        it.path("/sentence/has-sentences-after-on-other-court-appearance")
          .queryParam("sentenceUuids", createdSentence.sentenceUuid)
          .build()
      }
      .exchange()
      .expectStatus()
