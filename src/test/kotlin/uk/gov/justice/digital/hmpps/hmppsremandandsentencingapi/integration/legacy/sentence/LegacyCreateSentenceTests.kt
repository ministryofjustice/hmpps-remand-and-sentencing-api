...
class LegacyCreateSentenceTests : IntegrationTestBase() {
  ...
  fun `creating a sentence with FTR 56 recall sentence type uses the DPS recall sentence type bucket`() {
    ...
    val appearance = courtCaseCreated.appearances.first()
    val charge = appearance.charges.first()
    val legacySentence = DataCreator.legacyCreateSentence(chargeUuids = listOf(charge.chargeUuid), appearanceUuid = appearance.appearanceUuid, sentenceLegacyData = DataCreator.sentenceLegacyData(sentenceCalcType = "FTR_56", sentenceCategory = "2003"), returnToCustodyDate = LocalDate.of(2024, 1, 1))
    webTestClient
      .post()
      .uri("/legacy/sentence")
      .bodyValue(legacySentence)
