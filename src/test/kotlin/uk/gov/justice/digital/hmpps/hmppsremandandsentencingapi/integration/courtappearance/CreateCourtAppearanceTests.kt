...
import uk.gov.justice.digital.hmpps.hmppsremandandsentencingapi.legacy.controller.legacy.controller.dto.ChargeLegacyData
...
class CreateCourtAppearanceTests : IntegrationTestBase() {
  ...
  fun `create appearance in existing court case`() {
    ...
    val appearance = CreateCourtAppearance(
      courtCase.first, UUID.randomUUID(), UUID.randomUUID(), null, "COURT1", "GH123456789", LocalDate.now(), null, "REMAND", 1, null, null, listOf(charge), LocalDate.now().minusDays(7),
  ...
  fun `create appearance with consecutive to sentences`() {
    ...
    val appearance = CreateCourtAppearance(
      courtCase.first, UUID.randomUUID(), UUID.randomUUID(), UUID.fromString("62412083-9892-48c9-bf01-7864af4a8b3c"), "COURT1", "GH123456789", LocalDate.now(), null, "SENTENCE", 1,
      CreatePeriodLength(
        6,
        null,
        null,
