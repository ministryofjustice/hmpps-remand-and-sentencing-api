...
class LegacyGetTestCourtCaseTests : IntegrationTestBase() {

  @Test
  fun `get test legacy court case by uuid`() {
    ...
    webTestClient
      .get()
      .uri("/legacy/court-case/$uuid/test")
      .headers {
  ...
  fun `no court case exist for uuid results in not found`() {
    webTestClient
      .get()
      .uri("/legacy/court-case/${UUID.randomUUID()}/test")
      .headers {
        it.authToken(roles = listOf("ROLE_REMAND_AND_SENTENCING_COURT_CASE_RO"))
      }
      .exchange()
  ...
  fun `no token results in unauthorized`() {
    ...
    webTestClient
      .get()
      .uri("/legacy/court-case/${createdCase.first}/test")
      .exchange()
      .expectStatus()
      .isUnauthorized
  }
